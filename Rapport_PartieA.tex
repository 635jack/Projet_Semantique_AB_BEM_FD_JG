...
Ce projet a pour dessein l'extraction de connaissances structurées à partir d'un corpus de textes non structurés. La première étape, cruciale, est le \textbf{preprocessing}, qui transforme les données brutes en un format exploitable \cite{manning99}.
...
\begin{enumerate}
    ...
    \begin{itemize}
        \item Utilisation de la librairie \textbf{spaCy} \cite{spacy17}.
...
\textbf{Choix de la méthode} : Vectorisation \textbf{TF-IDF} (Term Frequency-Inverse Document Frequency) \cite{salton88}.
...
Prenons la phrase suivante du dataset (identifiant \texttt{en-doc6331-sent60}) relatant l'histoire du jeu vidéo \textit{Doom} :
\begin{quote}
...
\end{quote}
Pour ce document spécifique, l'analyse TF-IDF identifie les termes les plus pertinents suivants :
\begin{itemize}
    \item \textbf{doom} (score: 0.34) : Terme central et spécifique au sujet.
    \item \textbf{original} (score: 0.24) : Qualifie la version du jeu.
\end{itemize}
Cela démontre la capacité du modèle à extraire les mots-clés thématiques malgré la présence de nombreux mots de liaison.
...
\bibitem{spacy17}
Honnibal, M., \& Montani, I. (2017). spaCy 2: Natural language understanding with Bloom embeddings, convolutional neural networks and incremental parsing. \textit{To appear}.
